/* =============================================================================
 * c-drawer — reusable offcanvas/drawer for SITE-CMS modules.
 *
 *   API (window.SGDrawer):
 ...
 *
 *   Auto-bound triggers:
 *     [data-c-drawer-open="<id>"]   // any element opens that drawer on click
 *     [data-c-drawer-close]         // any descendant of .c-drawer-host
 ...
 *     - Body scroll-lock via the .c-drawer-locked class.
...
(function (window, document) {
    ...
    function lockScroll() { document.body.classList.add('c-drawer-locked'); }
    function unlockScroll() { if (!stack.length) document.body.classList.remove('c-drawer-locked'); }
    ...
    document.addEventListener('click', function (e) {
        var openTrigger = e.target.closest('[data-c-drawer-open]');
        if (openTrigger) {
            e.preventDefault();
            open(openTrigger.getAttribute('data-c-drawer-open'));
            return;
        }
        var closeTrigger = e.target.closest('[data-c-drawer-close]');
        if (closeTrigger) {
            // If the attribute has a value, treat it as an explicit target id;
            // otherwise close the parent drawer host.
            var explicit = closeTrigger.getAttribute('data-c-drawer-close');
            if (explicit) {
                close(explicit);
            } else {
                var host = closeTrigger.closest('.c-drawer-host');
